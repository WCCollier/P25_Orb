...
- **Processing body** — transceiver, FPGA bridge, compute, LNAs, GNSS, secure element, Ethernet, PSU. Identical for every band. **Correction [2026-07-31]:** this previously read "the transceiver tunes 70 MHz–6 GHz," which was true of the AD9361 but is false of the ADRV9026 selected on the same day — that part tunes **650 MHz–6 GHz**, leaving VHF and UHF below its floor. The proposed fix is block up-conversion inside those two band modules, which keeps the body band-agnostic. Open item J, `docs/hardware-design.md` §3.3 and §7.4.
