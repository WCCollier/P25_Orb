...
**Shared with the Orb:** the transceiver family (ADRV9002 — four devices carry **eight transmit chains**, comfortably enough for a control channel plus voice channels), the FPGA and compute stack, the P25 protocol software, and pairing with ARC Edge.
...
**The RF Environment pane of the demo maps to a real product surface** — a diagnostics sub-view within the Control Panel, useful for the deploying technician to confirm the unit is hearing the target trunk before walking away during setup.
...
**Off-the-shelf: RF transceiver silicon [selected 2026-07-31, after two reversals the same day].** **Four × Analog Devices ADRV9002** — 30 MHz–6 GHz, two receivers each, two independent RF synthesisers per device, on-chip multichip synchronisation, 150 dBc/Hz dynamic range **held across the gain-control range**, and an ordinary LVDS/CMOS SSI digital interface rather than JESD204B/C. No custom silicon needed. The eight chains are arranged as **two coherent groups** — three elements plus a calibration reference each: one group on the **uplink** (~806–808 MHz, where handsets transmit, and therefore where the bearings that matter come from), one on the **downlink** (~851–853 MHz: the control channel, `8TAC95D` talkaround, and a known-position reference). **Fallback: two synchronised AD9361s** on one unified 47 MHz window, documented in `docs/hardware-design.md` §5.5.

**Why the selection moved twice in one day.** A three-element array needs three coherent chains; the AD9361 has two per die, so it takes two devices and a multi-chip synchronisation procedure at every start-up — and **a sync that half-succeeds produces a *wrong bearing rather than a missing one***, the silent-confident-error class this design works throughout to eliminate. That argument pointed at the ADRV9026, and it was wrong for a reason nobody had checked: the ADRV9026 tunes only from 650 MHz, putting the VHF and UHF band modules below its floor. **The second and much larger finding was that the array had been pointed at the wrong half of the band.** The captured slice was the *downlink*, so every bearing the system could compute would have been a bearing to the tower — the one transmitter whose position is never in question. Handsets transmit on the uplink and on talkaround. Both reversals are recorded in `docs/hardware-design.md` §5.5 and §7.4 item J.

The ADRV9002 answers all of it. Its tuning range covers every band in the Texas plan with no up-conversion, no mixers and no added spurs. Multichip synchronisation is required *within* a single device as well as between devices — each of its two receivers has its own synthesiser — so **device count stopped being a reliability variable**, which is what dissolved the original argument against the AD9361. And two narrow windows give **two independent gain controls**: a patrol car keying up on the uplink thirty metres away can no longer desensitise the downlink control channel, which is the entire event stream and the source of every alarm the product raises. Costs: four devices rather than one, three element splitters, two calibration tones, and a continuous-receive power figure that has not been measured.
...
**Scope:** real, working logic for multi-Orb direction-finding, generated as a standalone artifact — written explanation, real runnable code, a small standalone test of its own. **Not integrated into the live demo**, which simulates a single P25 Orb doing basic transcription/classification only. **Priority: explicitly lower than the core demo** — first thing to trim if effort/budget gets tight.
...
### Single-tab structure — Presenter View

The demo runs in one browser tab, `presenter.html`, framing two panes side by side:

- **RF Environment pane — ground truth.** Plays a scripted timeline of simulated P25 events (Channel Request, Grant, Queue, Deny, garbled/cut-off transmissions) — raw chaos, no interpretation.
- **Command Feed pane — OSC's ARC Edge control panel.** The same events, delivered as ARC Edge actually presents them: live digest feed, two-tier alarm panel, command view. Collapsible to a thin rail when the presenter wants the audience's full attention on the Command Feed alone; collapsing narrows the pane rather than unloading it, so its live connection never drops.

Both driven by the same underlying scripted event timeline. The RF Environment pane is the source of truth, broadcasting each event to the Command Feed pane in real time via the browser's `BroadcastChannel` API — the Command Feed genuinely reacts live, not a separately-timed replay.

This started as two separate browser tabs, each an independent top-level page. Two problems ended that: first, `BroadcastChannel` is scoped only by channel name, with no session identity, so a second pairing on the same channel (e.g. a stray tab left open from earlier testing) cross-talks with the first and corrupts its state. Second, and more fundamental, browsers throttle or fully suspend `requestAnimationFrame` — the sole driver of the RF Environment's playback clock — in a tab that isn't currently foregrounded, so switching focus from the RF Environment tab to the Command Feed tab silently froze the simulation the audience was supposed to be watching react. Framing both pages as same-origin iframes inside one always-visible tab resolves both: Page Visibility throttling keys off the top-level document, not individual iframes within it, so neither pane is ever backgrounded regardless of which one has the presenter's attention; and as long as only one Presenter View tab is open, there is only ever one pairing on the channel. `rf-environment.html` and `command-feed.html` still exist as the two underlying pages — Presenter View is how they're loaded, not a replacement for them — but there is no supported way to open either of them standalone anymore.
...
7. Resolution — the Command view shows the synthesized picture reaching the OSC, despite the chaos in the RF Environment pane.
...
**Simulated, but not because of any ARC Edge access limitation — no radio hardware exists for this demo regardless:** the RF Environment pane's event feed is a scripted timeline, using accurate P25 terminology and data shapes, feeding the real detection engine.
...
| **B** | Third coherent receive chain + uplink coverage | **DECIDED — 4× ADRV9002 in two coherent groups.** Eight chains: three elements plus a calibration reference per group, uplink and downlink captured simultaneously. Two synchronised AD9361s on one wide window are the fallback. Decided on reliability, not a completed costing |
...
**G — power, resolved, and it unlocked B.** The deployment assumption is a few hundred watts from a patrol vehicle or field power bank. Against a receive-only draw in the tens of watts that is comfortable. **The order matters: the power envelope had to settle before a multi-device receiver could win on reliability**, because 5–7 W was a serious objection against a backpack battery and is not one against a vehicle supply. The four-device figure is larger again and remains unmeasured — see `docs/hardware-design.md` §6. It also changes the receive-only argument from a feasibility claim to an endurance one.
